...
export default function PropertyPage({ params }: { params: Promise<{ hostId: string; propertyId: string }> }) {
  ...
  const handleApplyCoupon = () => {
    ...
    setCouponSuccess(`¡Cupón aplicado! ${found.type === "percent" ? `${found.amount}% de descuento` : `${formatMoney(found.amount, property?.currency)} de descuento`}`);
  };
        ...
        <div className="max-w-md w-full text-center space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700">
          ...
          <div className="bg-white rounded-3xl border border-slate-200 shadow-lg p-6 text-left space-y-3">
            ...
            <div className="border-t pt-3 flex justify-between font-extrabold text-base">
              ...
              <span className="text-slate-900">{formatMoney(bookingConfirmed.total, property?.currency)}</span>
            </div>
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="text-right flex flex-col items-end">
                        <span className="font-bold text-slate-900">{formatMoney(upsell.price, upsell.currency ?? property?.currency)}</span>
                        {isSelected
            ...
            <div className="sticky top-32">
              <Card className="border border-slate-200 shadow-2xl rounded-3xl overflow-hidden p-6 relative">
                ...
                <div className="flex justify-between items-end mb-6">
                  <div>
                    <span className="text-3xl font-extrabold text-slate-900">{formatMoney(basePrice, property?.currency)}</span>
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="bg-amber-50 rounded-2xl p-4 border border-amber-100 flex items-center justify-between">
                ...
                <div className="text-right">
                  ...
                  <p className="text-2xl font-extrabold text-amber-600">{formatMoney(finalTotal, property?.currency)}</p>
                </div>
              </div>
                  ...
                  {selectedUpsellIds.map(id => {
                    ...
                    return u ? (
                      <div key={id} className="flex justify-between text-sm text-slate-700">
                        ...
                        <span className="font-bold">{formatMoney(u.price, u.currency ?? property?.currency)}</span>
                      </div>
                    ) : null;
                  })}
                </div>
              ...
              <div className="bg-slate-50 rounded-2xl p-4 border border-slate-200 space-y-2 text-sm">
                <div className="flex justify-between text-slate-600"><span>{formatMoney(basePrice, property?.currency)} × {Math.max(nights, 1)} noches</span><span>{formatMoney(stayTotal, property?.currency)}</span></div>
                {upsellsTotal > 0 && <div className="flex justify-between text-slate-600"><span>Servicios extra</span><span>{formatMoney(upsellsTotal, property?.currency)}</span></div>}
                {discount > 0 && <div className="flex justify-between text-green-600"><span>Descuento ({appliedCoupon?.code})</span><span>−{formatMoney(discount, property?.currency)}</span></div>}
                <div className="flex justify-between text-slate-600"><span>Limpieza</span><span>{formatMoney(cleaningFee, property?.currency)}</span></div>
                <div className="flex justify-between text-slate-600"><span>Impuestos (16%)</span><span>{formatMoney(taxes, property?.currency)}</span></div>
                {processingFee > 0 && (
                  <div className="flex justify-between text-slate-600">
                    <span>Comisión de procesamiento ({processingFeePercent}%)</span>
                    <span>{formatMoney(processingFee, property?.currency)}</span>
                  </div>
                )}
                <div className="flex justify-between font-extrabold text-base text-slate-900 border-t pt-2 mt-2">
                  <span>Total</span><span>{formatMoney(finalTotal, property?.currency)}</span>
                </div>
              </div>
              ...
              >
                {isSubmitting ? (
                  ...
                  `Pagar y confirmar reserva · ${formatMoney(finalTotal, property?.currency)}`
                ) : (
