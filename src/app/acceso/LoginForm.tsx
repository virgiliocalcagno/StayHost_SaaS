 ...
 *
 * El gating server-side esta en `page.tsx` — si llegamos aca es porque
 * el server confirmo que NO hay sesion activa. Solo nos toca capturar
 * credenciales y disparar `signInWithPassword`.
 *
 * Despues del login forzamos full page load (`window.location.assign`)
 * para que el middleware corra con la cookie nueva. Una soft navigation
 * podria servir RSC cacheado y dejar al usuario "logueado" en cliente
 * pero rebotado por el siguiente request protegido.
 */
export function LoginForm({ initialEmail }: { initialEmail: string }) {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if (authError) {
        // Supabase devuelve "Invalid login credentials" para email Y password
        // incorrectos — intencional, para no leak cual de los dos fallo.
        setError(
