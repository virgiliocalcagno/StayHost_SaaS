 ...
 *
 * Credentials are read exclusively from server env vars. Callers used to be
 * able to pass them in the body, which was unsafe — that path has been
 * removed.
 *
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const { action, accessToken, deviceId, ...params } = payload;
    // Ignore any credentials smuggled in the body — we only trust env vars.
    ...
    if (!clientId) {
      return NextResponse.json({
        mock: true,
        message: "Tuya credentials not configured. Using demo mode.",
        data: getMockData(String(action), String(deviceId ?? "")),
      });
    }

    switch (action) {
      case "getToken": {
        const data = await tuyaRequest("GET", "/v1.0/token?grant_type=1", null);
      ...
      case "getAuthData": {
        ...
        const authUrl = `https://iot.tuya.com/cloud/authorize?client_id=${clientId}&redirect_uri=${encodeURIComponent(
          String(redirectUri)
        )}&response_type=code&region=${region}`;
      ...
      case "listDevices": {
        ...
        if (!uid) {
          const data = await tuyaRequest("GET", "/v1.0/iot-03/devices?page_size=50", null, String(accessToken));
          return NextResponse.json(data);
        }
        const data = await tuyaRequest("GET", `/v1.0/users/${uid}/devices`, null, String(accessToken));
        return NextResponse.json(data);
      }
      case "listAllDevices": {
        const data = await tuyaRequest("GET", "/v1.0/iot-03/devices?page_size=50", null, String(accessToken));
      ...
      case "getDevice": {
        const data = await tuyaRequest("GET", `/v1.0/devices/${deviceId}`, null, String(accessToken));
        return NextResponse.json(data);
      }
      case "getStatus": {
        const data = await tuyaRequest("GET", `/v1.0/devices/${deviceId}/status`, null, String(accessToken));
        return NextResponse.json(data);
      }
      case "sendCommand": {
      ...
      case "getLogs": {
        const data = await tuyaRequest("GET", `/v1.0/devices/${deviceId}/logs?type=7&size=20`, null, String(accessToken));
        return NextResponse.json(data);
      }
      default:
      ...
      return {
        result: [
          ...
          },
          {
            ...
            online: true,
            status: [
              { code: "va_temperature", value: 289 },
              { code: "va_humidity", value: 65 },
            ],
          },
        ],
      };
