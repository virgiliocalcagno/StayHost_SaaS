...
export async function POST(req: NextRequest) {
  ...
  const { data: bk } = await supabaseAdmin
    .from("bookings")
    .select("id, tenant_id, property_id, status, paid_at, payment_method, check_in, check_out, guest_name, guest_phone, guest_doc, guest_nationality, guest_doc_photo_path, phone_last4, channel_code")
